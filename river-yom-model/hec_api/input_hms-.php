...
// --- รวมข้อมูลจาก API ทั้งสองแหล่งเข้าด้วยกัน ---
$all_rain_data_by_station_date = [];
foreach ($rid_data_raw as $record) {
    if (isset($record['sta_code']) && isset($record['date'])) {
        $key = $record['sta_code'] . '_' . $record['date'];
        $all_rain_data_by_station_date[$key] = floatval($record['rain_mm'] ?? 0);
    }
}
