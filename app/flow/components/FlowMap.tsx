...
const FlowMap: React.FC<LongdoMapProps> = ({mapKey, stationType, JsonPaths ,height }) => {
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/flow_data_last_14_days`)
      .then(res => res.json())
      .then(result => {
        if (result.status === 'success') {
          const rawData: FlowDataItem[] = result.data;
  
          // เก็บข้อมูลทั้งหมด
          setFlowApiData(rawData);
  
          // สร้าง Map เพื่อเก็บเฉพาะรายการล่าสุดต่อ sta_code
          const latestDataMap = new Map<string, FlowDataItem>();
  ...
  useEffect(() => {
    if (isMapReady) {
      ...
      const addMarkers = async () => {
        try {
          if (flowData.length > 0) await addMarkersFromFlowData();
  ...
  }, [isMapReady, flowData]); // รอให้ข้อมูลทั้งหมดพร้อมก่อน
...
const prepareChartDataForFlow = (rawData: any[], targetStaCode: string) => {
  ...
  const renderChartPart = (sta_code: string, type: 'discharge' | 'wl') => {
    ...
    // เตรียมข้อมูล
    const raw = flowApiData.filter((d: any) => d.sta_code === sta_code);
    const chartData = prepareChartDataForFlow(raw, sta_code);
    let seriesData = [];
    let title = '';
    let color = '';
  ...
  const handleListItemClick = (item: any, type: "flow") => {
    ...
    let icon = "";
      ...
      `;
      icon = `${Path_URL}images/icons/flow_station_icon.png`;
