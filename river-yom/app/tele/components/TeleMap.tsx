...
const TeleMap: React.FC<LongdoMapProps> = ({mapKey, stationType, JsonPaths ,height }) => {
  ...
  const [teleApiData, setTeleApiData] = useState<TeleDataItem[]>([]);
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/tele_data_last_14_days`)
      .then(res => res.json())
      .then(result => {
        if (result.status === 'success') {
          const rawData: TeleDataItem[] = result.data;
  
          // เก็บข้อมูลทั้งหมด
          setTeleApiData(rawData);
  
          // สร้าง Map เพื่อเก็บเฉพาะรายการล่าสุดต่อ sta_code
          const latestDataMap = new Map<string, TeleDataItem>();
  ...
  useEffect(() => {
    if (isMapReady) {
      ...
      console.log("🏞️ teleData:", teleData);
      const addMarkers = async () => {
        try {
          if (teleData.length > 0) await addMarkersFromTeleData();
  ...
  }, [isMapReady, teleData]); // รอให้ข้อมูลทั้งหมดพร้อมก่อน
...
const prepareChartDataForTele = (rawData: any[], targetStaCode: string) => {
  ...
  const renderChartPart = (sta_code: string, type: 'discharge' | 'wl') => {
    ...
    // เตรียมข้อมูล
    const raw = teleApiData.filter((d: any) => d.sta_code === sta_code);
    const chartData = prepareChartDataForTele(raw, sta_code);
    let seriesData = [];
    let title = '';
    let color = '';
  ...
  const latestDataMap =
  Array.isArray(latestTeleData) && latestTeleData.length > 0
    ? new Map(latestTeleData.map((d) => [d.sta_code, d]))
    : new Map();


  const addMarkersFromTeleData = () => {
    if (!map) return;

    let newMarkers: any[] = [];

    teleData.forEach((data) => {
    ...
    console.log("✅ เพิ่ม markers จาก Tele data");
  };

  const handleListItemClick = (item: any, type: "tele") => {
      ...
      detail = `
           <div style="font-family: 'Prompt', sans-serif; font-size: 1rem;">
            ...
            <div style="font-family: 'Prompt', sans-serif; font-size: 0.9rem; line-height: 1.4rem;">
              ...
              <a href="/river-yom/tele?tab=0&station=${item.sta_code}" 
                style="padding: 4px 10px;  font-family: 'Prompt', sans-serif; background-color: #1976d2; color: white; border-radius: 6px; text-decoration: none; font-size: 0.9rem; display: inline-block; margin-top: 8px; cursor: pointer; transition: background-color 0.2s;">
                 ข้อมูลเพิ่มเติม
              </a>
    ...
    const marker = new longdo.Popup(position, {
      ...
      icon: {
        html: `<div style="text-align:center;">
          ...
          ${type === "tele" ? `${item.sta_code}` : item.sta_name}</div></div>`
      },
      size: { width: 450, height: 'auto' },
