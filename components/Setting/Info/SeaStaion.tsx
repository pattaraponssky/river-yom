...
const InfoSeaStation: React.FC = () => {
  ...
  const handleEditSave = () => {
    fetch(`${API_URL}/api/sea_info/${editFeature.sta_code}`, {
      method: "PUT",
  ...
  const handleDeleteConfirm = () => {
    fetch(`${API_URL}/api/sea_info/${deleteStaCode}`, { method: "DELETE" })
      .then((res) => res.json())
      .then(() => {
  ...
  const handleAddFeature = () => {
    if (!newFeature.sta_code || !newFeature.name) {
  ...
  return (
    <Container component="main" sx={{ minWidth: "100%" }}>
      ...
      <Dialog open={openAddDialog} onClose={() => setOpenAddDialog(false)} fullWidth maxWidth="md">
        <DialogTitle sx={{ fontFamily: "Prompt", fontWeight: "bold" }}>เพิ่มสถานีน้ำฝน</DialogTitle>
        <DialogContent>
