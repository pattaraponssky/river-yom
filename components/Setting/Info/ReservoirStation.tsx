...
const InfoDamStation: React.FC = () => {
  ...
  const [deleteResCode, setDeleteResCode] = useState<string>("");
  ...
  const handleEditSave = () => {
    fetch(`${API_URL}/api/reservoir_info/${editFeature.res_code}`, {
      method: "PUT",
      credentials: 'include',
      ...
      .then(() => {
        const updated = stations.map((s) =>
          s.res_code === editFeature.res_code ? editFeature : s
        );
        setStations(updated);
  ...
  const handleDeleteConfirm = () => {
    fetch(`${API_URL}/api/reservoir_info/${deleteResCode}`, { method: "DELETE" ,  credentials: 'include',})
      .then((res) => res.json())
      .then(() => {
        setStations(stations.filter((s) => s.res_code !== deleteResCode));
  ...
  const handleAddFeature = () => {
    if (!newFeature.res_code || !newFeature.res_name) {
  ...
  return (
    <Container component="main" sx={{ minWidth: "100%" }}>
      ...
      <Dialog open={openEditDialog} onClose={() => setOpenEditDialog(false)} fullWidth maxWidth="md">
        <DialogTitle sx={{ fontFamily: "Prompt", fontWeight: "bold" }}>แก้ไขสถานี {editFeature.res_code}</DialogTitle>
        <DialogContent>
          <Box sx={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: 2, mt: 2 }}>
      ...
      <Dialog open={openDeleteDialog} onClose={() => setOpenDeleteDialog(false)}>
        ...
        <DialogContent>
          <Typography>
            คุณต้องการลบข้อมูลสถานีรหัส <strong>"{deleteResCode}"</strong> ใช่หรือไม่?
          </Typography>
        </DialogContent>
        <DialogActions>
