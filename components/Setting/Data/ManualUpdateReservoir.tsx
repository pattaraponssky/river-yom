...
const ManualUpdateReservoir: React.FC = () => {
  ...
  const handleUpdate = async () => {
    ...
    // 3. Construct API URL
    const apiUrl = `${API_URL}/jobs/updateReservoirFillData/${formattedStartDate}/${formattedEndDate}`;
    ...
    try {
      ...
      if (response.ok) {
        setSnackbarMessage(
          data.message || `อัปเดตข้อมูลอ่างเก็บน้ำสำเร็จสำหรับช่วง ${formattedStartDate} ถึง ${formattedEndDate}`
        );
      ...
      console.error("Error updating reservoir data:", error);
  ...
  return (
    <Box sx={{ fontFamily: "'Prompt', sans-serif", mt: {md:4,xs:2} }}>
      <Typography variant="h5" sx={{ marginBottom: "1rem", fontWeight: 600, ...titleStyle, color: "#28378B" }}>
        อัปเดตข้อมูลอ่างเก็บน้ำย้อนหลัง (กำหนดวันที่)
      </Typography>
      <Paper sx={{ p: { xs: 2, sm: 4 }, borderRadius: 2, backgroundColor: "#f0f0f0" }}>
        <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={th}> {/* Use Thai locale */}
        ...
        <Button
          ...
          disabled={loading || !startDate || !endDate} // Disable if loading or dates not selected
          sx={{
            ...titleStyle,
            borderRadius: 2,
...
export default ManualUpdateReservoir;